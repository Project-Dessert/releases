#!/bin/bash

export GITHUB_USER="SirRGB"
export GITHUB_EMAIL="sirrgb@proton.me"

export device="griffin"

export ROM="Project Dessert"
export ROM_DIR="${WORKSPACE}/../Des12"
export ROM_VERSION="12L"
export official="false"
export local_manifest_url="https://raw.githubusercontent.com/SirRGB/local_manifests/main/griffin/A12Dessert.xml"
export manifest_url="https://github.com/Project-Dessert/android"
export rom_vendor_name="lineage"
export branch="snowcone"
export bacon="bacon"
export buildtype="eng"
export clean="installclean"
export generate_incremental="false"
export upload_recovery="false"

export ccache="false"
export ccache_size=""

export jenkins="false"

export release_repo="Project-Dessert/releases"

export timezone="UTC"
